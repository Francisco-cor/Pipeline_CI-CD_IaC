...
SQS (ordenes queue + DLQ Fase 10.6) ← ordenes publish, stock consume opcional (aún no iniciado por defecto)
```
...
- **Queue:** `queue.js:1` `@aws-sdk/client-sqs` `SQS_QUEUE_URL` + `sqs.tf:1` `ordenes` + DLQ (10.6) + `ordenes.js:140` `publishOrdenCreada` async best-effort. El polling `startConsumer` existe como componente, pero no se arranca automáticamente.
